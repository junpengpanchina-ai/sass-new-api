...
![支付设置页面](./assets/setting-payment.png)
...
> 警告（合规提示）：网关本身不等同于持牌支付机构；资金清结算与合规依赖其对接的持牌渠道，请遵循所在地监管与风控要求。
...
![Stripe 配置](./assets/setting-payment-stripe.png)
...
> 提示：Stripe 需要配置 Webhook 接收支付状态通知，Webhook URL 为 `https://your-domain.com/api/payment/stripe/webhook`。
...
> 提示：充值折扣可以激励用户一次性充值更多金额，提高用户粘性。

## 限流设置

配置 API 调用的频率限制，防止滥用和保护系统稳定性。

![限流设置页面](./assets/setting-rate-limit.png)
...
> 警告：限流设置过低可能影响正常使用，建议根据实际业务需求合理配置。
...
在 New API 系统中，倍率是计算配额消耗的关键参数。配额是系统内部的计费单位，所有的 API 调用最终都会转换为配额点数进行扣减。
...
模型倍率定义了不同 AI 模型的基础计费倍数，系统为各种模型预设了默认倍率。
...
![模型倍率设置 - 页面1](./assets/setting-ratio-model-1.png)

1. 在系统设置页点击「倍率设置」标签页
2. 在模型倍率列表中找到目标模型

![模型倍率设置 - 页面2](./assets/setting-ratio-model-2.png)
...
![模型倍率设置 - 页面3](./assets/setting-ratio-model-3.png)
...
| gpt-3.5-turbo | 0.5$/1M Tokens | 1$/1M Tokens | 2 | 输出是输入的 2 倍 |
...
![分组倍率设置 - 页面4](./assets/setting-ratio-group-1.png)
...
![分组倍率设置 - 页面5](./assets/setting-ratio-group-2.png)
...
> 提示：有关更多计费规则，请查看 [常见问题](/zh/docs/support/faq)。

## 聊天设置

配置内置聊天功能的相关参数。

![聊天设置页面](./assets/setting-chat.png)
...
![绘图设置页面](./assets/setting-image.png)
...
> 提示：Midjourney 功能需要额外部署 Midjourney-Proxy 服务，详见部署文档。
...
![数据看板设置 - 页面1](./assets/setting-dashboard-1.png)
...
![数据看板设置 - 页面2](./assets/setting-dashboard-2.png)
...
![数据看板设置 - 页面3](./assets/setting-dashboard-3.png)
...
![模型设置 - 页面1](./assets/setting-model-1.png)
...
![模型设置 - 页面2](./assets/setting-model-2.png)
...
![模型设置 - 页面3](./assets/setting-model-3.png)
...
![运营设置 - 页面1](./assets/setting-ops-1.png)
...
![运营设置 - 页面2](./assets/setting-ops-2.png)
...
![运营设置 - 页面3](./assets/setting-ops-3.png)
...
![其他设置 - 页面1](./assets/setting-misc-1.png)
...
![其他设置 - 页面2](./assets/setting-misc-2.png)
...
> 提示：以上所有设置仅 Root 用户可见和修改，普通管理员无权访问。
