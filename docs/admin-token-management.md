...
> 仓库内当前实现：已提供 `/console/token` 页面，包含「令牌列表 / 创建弹窗 / 创建后仅显示一次完整 Key / 编辑 / 删除」。配额消耗与网关权限校验后续接入。

## 令牌列表

> 截图占位：`docs/assets/token-list.png`
...
> 截图占位：`docs/assets/token-create-modal.png`
...
点击「提交」，弹窗显示完整的令牌 Key，**立即复制保存**，关闭提示后无法再次查看完整 Key。

> 截图占位：`docs/assets/token-created-key.png`

注意：

- 令牌 Key 仅在创建时完整显示一次，请立即复制保存。
- 令牌 Key 具有完整的 API 调用权限，请勿泄露给他人，不要提交到代码仓库。
